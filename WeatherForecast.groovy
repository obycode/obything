...
definition(
    ...
    description: "Play a weather report through your AirPlay speakers when the mode changes or other events occur",
...
private speakerOptions() {
    ...
    // Query for speakers
    def states = sonos.statesSince("speakers", new Date(0), [max:30])
    log.debug("states are: ${states}")
    def dataMaps = states.collect{it.value}
    options.addAll(dataMaps.collect{it})
...
private takeAction(evt) {
    ...
    if (song) {
        sonos.playSoundAndTrack(state.sound.uri, state.sound.duration, state.selectedSong, volume, speakers)
    }
    else if (resumePlaying){
        sonos.playTrackAndResume(state.sound.uri, state.sound.duration, volume, speakers)
    }
    else if (volume) {
        sonos.playTrackAtVolume(state.sound.uri, volume, speakers)
    }
    else if (speakers) {
        sonos.playTrackOnSpeakers(state.sound.uri, speakers)
    }
    else {
